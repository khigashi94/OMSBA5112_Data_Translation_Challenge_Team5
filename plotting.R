...
plot <- select(agri_plot, new_HHID, s8bq4a,s8bq12a, s8bq4b, s8bq5, s8bq12a, s8bq12b) 
...
crop_combined_distinct <- distinct(crop_combined,cassava, maize, unknown, pawpaw, new_HHID)
...
##############################################################################################
############## Plots of Highest Level and Savings Residuals ##################################
##############################################################################################

## highest_level_avgs
highest_level_avgs_lm <- lm(Final_df, formula = sum_aggr_inc ~ highest_level_avgs)

summary(highest_level_avgs_lm)
highest_level_avgs_lm
hist(rstandard(highest_level_avgs_lm ), # normal distribution of errors?
     xlab = "Standardized residuals")

plot(fitted(highest_level_avgs_lm ), resid(highest_level_avgs_lm ),
     xlab = "Fitted", ylab = "Residuals",
     main = 'Fitted vs Residuals for Average Highest Level Education in Household ',
     abline(h = 0, col = "blue"))

 

## savings_val
savings_vals_lm <- lm(Final_df, formula = sum_aggr_inc ~ savings_val)

summary(savings_vals_lm)

hist(rstandard(savings_vals_lm ), # normal distribution of errors?
     xlab = "Standardized residuals")

plot(fitted(savings_vals_lm ), resid(savings_vals_lm ),
     xlab = "Fitted", ylab = "Residuals",
     main = 'Fitted vs Residuals for Monetary Savings in Household ',
     abline(h = 0, col = "blue"))
